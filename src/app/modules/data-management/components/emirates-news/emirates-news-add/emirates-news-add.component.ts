...
@Component({
  ...
  styleUrls: ['./emirates-news-add.component.scss']
})
export class EmiratesNewsAddComponent implements OnInit {
  ...
  constructor(
    ...
    private latestNewsService: EmiratesNewsService,
    ...
    if (this.form.valid) {
      ...
      .subscribe((response) => {
        if (response.IsSuccess) {
          ...
          this.fileManagerService.upload(id, 'EmiratesNews','', [this.form.get('Image')?.value]).subscribe(res =>{
