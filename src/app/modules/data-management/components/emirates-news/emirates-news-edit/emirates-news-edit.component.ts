...
@Component({
  ...
  styleUrls: ['./emirates-news-edit.component.scss']
})
export class EmiratesNewsEditComponent implements OnInit {
  ...
  getEdit(id) {
    this.emiratesNewsService.getById(id).subscribe(
      (response) => {
        debugger;
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.emiratesNewsService.update(postedVM)
      .subscribe((response) => {
        if (response.IsSuccess) {
          debugger;
          if(this.form.get('Image')?.value){
            this.fileManagerService.deleteByEntityName(this.editVM.Id,'EmiratesNews').subscribe(res=>{
              this.fileManagerService.upload(this.editVM.Id, 'EmiratesNews','', [this.form.get('Image')?.value]).subscribe(res =>{
                this.globalService.messageAlert(MessageType.Success,'تم التعديل بنجاح');
                this.router.navigate(["/data-management/emirates-news-list"]);
              })
            })
          } else{
            this.router.navigate(["/data-management/emirates-news-list"]);
