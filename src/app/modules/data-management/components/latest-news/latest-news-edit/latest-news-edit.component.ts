...
export class LatestNewsEditComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.latestNewsService.update(postedVM)
      .subscribe((response) => {
        if (response.IsSuccess) {
          debugger;
          if(this.form.get('Image').value){
            this.fileManagerService.deleteByEntityName(this.editVM.Id,'LatestNews').subscribe(res=>{
              this.fileManagerService.upload(this.editVM.Id, 'LatestNews','', [this.form.get('Image').value]).subscribe(res =>{
